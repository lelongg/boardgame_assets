import type { CardData, CardTemplate, CardTemplateSection, CardTemplateItem, AnchorPoint, CardTemplateTextItem, CardTemplateFrameItem, CardTemplateImageItem } from "./types.js";
...
const normalizeAnchorPoint = (anchor: unknown): AnchorPoint => {
  ...
  const normalizeCoord = (coord: unknown): 0 | 0.5 | 1 => {
    ...
    if (num >= 0.75) return 1;
    if (num >= 0.25) return 0.5;
    return 0;
  };
  return {
...
const normalizeItem = (item: unknown): CardTemplateItem => {
  ...
  const type = hasType ? safeEnum(obj.type, ["text", "frame", "image"] as const, "text") : null;
  ...
  if (type === "image") {
    const imageItem: CardTemplateImageItem = {
      ...
      fit: obj.fit !== undefined && obj.fit !== null ? safeEnum(obj.fit, ["cover", "contain", "fill"] as const, "cover") : undefined,
  ...
  const textItem: CardTemplateTextItem = {
    ...
    align: safeEnum(obj.align, ["left", "center", "right"] as const, "left"),
    font: obj.font !== undefined && obj.font !== null ? safeEnum(obj.font, ["title", "body"] as const, "body") : undefined,
...
const normalizeSection = (section: unknown): CardTemplateSection => {
  ...
  const layout = safeEnum(obj.layout, ["row", "column", "stack"] as const, "stack");
...
export const normalizeCard = (card: unknown): CardData => {
  ...
  return {
    ...
    fields: Object.fromEntries(
      Object.entries(fields).map(([key, value]) => [key, safeString(value, "")])
    )
  };
};
